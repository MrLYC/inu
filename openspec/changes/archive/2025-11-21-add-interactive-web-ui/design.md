...
7. 成功后：右侧面板显示脱敏文本，"切换到还原模式"按钮出现
8. JavaScript 将实体和脱敏文本存储到 sessionStorage

**还原流程:**
1. 用户点击"切换到还原模式"
2. UI 切换到视图 2（还原模式）
3. 顶部显示实体映射，左侧面板显示脱敏文本（只读）
4. 用户将外部处理的文本粘贴到右侧面板
5. 用户点击"还原" → 出现加载动画
...
- 懒加载：仅在需要时加载视图组件
- 防抖：输入后等待 500ms 再启用"脱敏"按钮（防止误点击）
