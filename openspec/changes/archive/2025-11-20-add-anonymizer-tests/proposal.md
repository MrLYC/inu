...
## Why
`pkg/anonymizer` 包是 Inu 的核心业务逻辑，负责文本匿名化和还原功能，但目前完全没有单元测试覆盖。这导致：
- 代码变更时无法快速验证核心逻辑的正确性
- 重构和优化存在较高风险
- CI/CD 中无法捕获匿名化逻辑的回归问题
- 新贡献者难以理解预期行为

由于匿名化功能依赖外部 LLM API，测试需要通过 mock 避免实际 API 调用，确保：
...
- 影响的代码:
  - 新增 `pkg/anonymizer/anonymizer_test.go`: 核心匿名化逻辑测试
