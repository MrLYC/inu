...
- [x] 1.1 在 `pkg/anonymizer/anonymizer.go` 文件顶部添加 `Anonymizer` 接口定义
...
- [x] 4.1 更新 `cmd/inu/commands/anonymize.go`
  - 导入保持不变
  - 调用 `anonymizer.NewHashHidePair(llm)` 替代 `anonymizer.New(llm)`
  - 变量类型保持 `anon` (Go 接口无需显式声明类型)
- [x] 4.2 更新 `cmd/inu/commands/restore.go`
  - 调用 `anonymizer.NewHashHidePair(llm)` 替代 `anonymizer.New(llm)`
- [x] 4.3 更新 `cmd/inu/commands/web.go`
  - 调用 `anonymizer.NewHashHidePair(llm)` 替代 `anonymizer.New(llm)`

## 5. Update Web Server
- [x] 5.1 更新 `pkg/web/server.go`
