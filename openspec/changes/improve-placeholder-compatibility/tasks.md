...
- [ ] 1.1 在 `pkg/anonymizer/anonymizer.go` 中添加 `normalizePlaceholder` 函数
  - 输入：原始占位符字符串（例如 `< 业务信息 [2]. 系统。名称 >`）
  - 输出：归一化后的占位符字符串（例如 `<业务信息[2].系统.名称>`）
  - 实现步骤：提取内容 → 移除空格 → 转换标点 → 转换全角/半角 → 重新包装
  
- [ ] 1.2 添加 `normalizePlaceholder` 单元测试到 `pkg/anonymizer/anonymizer_test.go`
  - 测试标准格式不变：`<个人信息[0].姓名.张三>` → `<个人信息[0].姓名.张三>`
  - 测试移除空格：`< 个人信息 [0]. 姓名. 张三 >` → `<个人信息[0].姓名.张三>`
...
- [ ] 2.2 添加 `RestoreText` 格式兼容性测试到 `pkg/anonymizer/anonymizer_test.go`
  - 测试标准格式仍然工作（向后兼容）
  - 测试还原包含额外空格的占位符
  - 测试还原包含中文标点的占位符
  - 测试还原包含全角字符的占位符
