...
- [x] 测试：脱敏"张三在阿里巴巴工作" → 看到"[PERSON_1]在[ORG_1]工作"

### 添加状态管理
- [x] 脱敏成功后在 sessionStorage 中存储 entities、anonymizedText、entityTypes
- [x] 添加"切换到还原模式"按钮（默认隐藏，脱敏后显示）
- [x] 测试：脱敏后刷新页面，验证状态恢复
...
### 视图切换逻辑
- [x] 添加 `switchToRestoreView()` 函数隐藏脱敏视图，显示还原视图
...
- [x] 添加网络错误处理（fetch 失败）
- [x] 测试：停止服务器，尝试脱敏，看到"无法连接到服务器"消息
