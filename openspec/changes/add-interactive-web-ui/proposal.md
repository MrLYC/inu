...
目前 `inu web` 仅提供 RESTful API，没有用户界面。用户必须通过 curl 或 Postman 等 API 客户端与脱敏服务交互。`inu interactive` 命令提供了更好的工作流程，具有跨多个还原周期的实体内存，但仅限于 CLI 环境。

**问题:**
- Web 模式缺少可视化界面，限制了可访问性
- 不熟悉 API 的用户无法轻松使用 Web 服务
- 交互式工作流（一次脱敏，多次还原）在 Web 模式下不可用
- 还原过程中缺少实体映射的可视化反馈
